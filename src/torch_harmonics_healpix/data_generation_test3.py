...
Reproduces the benchmark from Krachmalnicoff & Tomasi (2019) Section 6.2.
Uses realistic CAMB power spectra with varying optical depth τ ∈ [0.03, 0.08].

Paper approach: pre-generate 5000 CAMB spectra with different τ values,
then randomly choose one spectrum + seed for each map.
"""
...
def precompute_camb_spectra(
    n_spectra: int = N_CAMB_SPECTRA,
    lmax: int = LMAX,
    seed: int = 12345,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Pre-compute CAMB spectra for N different τ values (paper: 5000).
    ...
    Args:
        ...
        lmax: Maximum multipole.
        seed: Random seed for τ values.

    Returns:
        Tuple of (tau_values, cl_ee_array, cl_bb_array):
    ...
    return tau_values.astype(np.float32), cl_ee_array, cl_bb_array
...
def generate_test3_dataset(
    ...
    Uses pre-computed CAMB spectra (paper: 5000 spectra).
    Each map randomly picks one spectrum + a unique realization seed.
    ...
    tau_spectra, cl_ee_array, cl_bb_array = precompute_camb_spectra(
    ...
    tau_values = tau_spectra[spectrum_indices]
